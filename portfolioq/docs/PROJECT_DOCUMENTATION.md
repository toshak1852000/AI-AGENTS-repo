...
- **report_generation_node:** Generates rebalancing recommendations; calls `generate_report`; calls `evaluate_and_create_alerts`; updates Prometheus metrics; returns report_id and status completed/failed
...
- **Portfolios:** GET/POST /portfolios, GET/PUT/DELETE /portfolios/{id}, GET/POST/DELETE holdings, POST refresh-prices
...
- **run-all.sh:** Runs show-services.sh, check-ml-models.sh, validate-ml-models-e2e.sh in sequence (from portfolioq root). Optional BASE_URL argument.
...
| Scenario-Based Rebalancing Recommendations | rebalancing_service; holding_recommendations, sector_recommendations in report |
...
- **Services:** Backend http://localhost:8000, MLflow http://localhost:5001, Grafana http://localhost:3001 (admin / portfolioq_admin), Prometheus http://localhost:9090
- **Run all validations:** From `portfolioq`: `bash scripts/run-all.sh`
